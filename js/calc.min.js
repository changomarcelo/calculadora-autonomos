$(function() {
	...
	$('#deduccionEspecial').text('$'+ DEDUCCION_ESPECIAL);
	$('#deduccionConyugue').text('$'+ DEDUCCION_CONYUGUE);
	$('#deduccionHijo').text('$'+ DEDUCCION_HIJO);
	$('#gananciaNoImponible').text('$'+ GANANCIA_NO_IMPONIBLE);
	$('#deduccSegVidaMax').text('$'+ SEGURO_VIDA_MAX);
	$('#deduccSegRetMax').text('$'+ SEGURO_RETIRO_MAX);
	$('#porcDonaciones').text(PORCENTAJE_DONACIONES +'%');
	$('#porcCuotasMed').text(PORCENTAJE_CUOTAS_MEDICAS +'%');
	$('#porcHonorariosMed').text(PORCENTAJE_HONORARIOS_MEDICOS +'%');
	$('#deduccionIntereses').text('$'+ INTERESES_MAX);
...
function calcular_aut() {
	...
	var seguroRetiro = $('#seguroRetiro').val();
	...
	var cuotas_medicas = $('#cuotasMedicas').val();
	...
	var honorarios_medicos = $('#honorariosMedicos').val();
	...
	var deducciones_grales = $('#deducciones_grales').val();
	console.debug('deducciones generales: '+ deducciones_grales);
	...
	var ganancia_neta = ingresos - egresos - deducciones_grales - dg - DEDUCCION_ESPECIAL - GANANCIA_NO_IMPONIBLE - hijos*DEDUCCION_HIJO - conyugue*DEDUCCION_CONYUGUE;
	console.debug('ganancia neta: '+ ganancia_neta);

	$('#gananciaNeta').text('$'+ganancia_neta);	
	...
	for (i = 0; i < n; i++)
	{
		if (ganancia_neta >= minimos[i])
		{
			if (i > 0)
			{
				imp_acumulado = minimos[i] * porcentajes[i-1] / 100;
				console.debug('es el '+ porcentajes[i-1] +'% de $'+ minimos[i]);
				console.debug('impuesto acumulado: '+ imp_acumulado);	
			}
	...
	var impuesto = imp_acumulado + ((ganancia_neta - imp_acumulado) * porcentaje / 100);
	$('#impuestoAnual').text('$'+ impuesto);
